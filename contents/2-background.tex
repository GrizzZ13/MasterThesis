...
大语言模型（LLMs）的重大突破始于Transformer架构，该架构彻底改变了自然语言处理（NLP）领域，并成为当前几乎所有先进AI模型的基础。
一个标准的Transformer层由两个关键子层构成：注意力层（Attention）和前馈神经网络（Feed-Forward Network, FFN）。

Transformer架构的核心在于注意力机制（Attention），该机制允许模型在处理一个词元（token）时，
能够同时考虑输入序列中的所有其他词元，从而有效捕捉长距离依赖关系。
在 Attention 层中，每个输入向量 x 被线性变换为三个独立的向量：查询（Query, Q）、键（Key, K）和值（Value, V）。
通过计算Q与K之间的点积得到注意力分数，再经过softmax函数归一化后，作为权重对V进行加权求和，
然后经过输出投影（Out Projection, O）最终产生该层的输出，这使得模型能够动态地关注输入中最相关的信息。

FFN 层紧随 Attention 层之后，通常由两个线性变换（全连接层）和一个非线性激活函数构成。
具体而言，给定 Attention 层的输出 \( x \in \mathbb{R}^{d_{\text{model}}} \)，
FFN 首先通过一个从 \( d_{\text{model}} \) 到 \( d_{\text{ff}} \) 的线性映射（通常 \( d_{\text{ff}} = 4 \times d_{\text{model}} \)），
再施加如 GeLU 或 ReLU 等激活函数，随后通过第二个线性映射将其投影回原始维度 \( d_{\text{model}} \)。
形式化地，FFN 可表示为：
...
\( \sigma(\cdot) \) 为非线性激活函数。
尽管结构简单，FFN 层在 Transformer 中承担着对注意力机制提取的上下文信息进行非线性特征变换与增强的关键作用，
其参数量通常占整个 Transformer 层的绝大部分。此外，FFN 层在各位置上独立作用，不涉及跨 token 的交互，
因此可高度并行化，适合在 GPU 等硬件加速器上高效执行。
...
推理系统系统会将这些K和V矩阵缓存起来，形成所谓的“键值缓存”（KV Cache）。
...
为了应对这一挑战，现代大模型推理系统通常将整个推理过程划分为两个截然不同的阶段：预填充（Prefill）和解码（Decode）。
...
大模型推理的分布式部署依赖多种并行策略（如张量并行 TP、专家并行 EP 等），
...
为实现高效的 GPU 通信，现代 GPU 架构引入了 NVLink 与 GPU 直接远程直接内存访问（GPU Direct RDMA, GDR）技术，
实现了纵向扩展（scale-up）网络与横向扩展（scale-out）网络的超高通信性能。

\subsection{NVLink}

NVLink 是 NVIDIA 开发的一种高性能芯片间互连技术，
...
GPU Direct RDMA 是 NVIDIA 为提升多节点（multi-node）GPU 系统通信效率而推出的一项关键技术。
...
包括使用支持 GDR 的 GPU 驱动（通常要求 R470 或更高版本）、兼容的 RDMA 网卡（如 Mellanox ConnectX-6/7）、启用 IOMMU/ATS 支持的操作系统配置，
以及应用程序或通信库对 GDR API 的显式或隐式调用。
随着 UCX（Unified Communication X）等统一通信框架的普及，GDR 的部署门槛正逐步降低，已成为现代 AI 集群的标配通信能力。

...
本节将聚焦张量并行（TP）、专家并行（EP）及预填充-解码分离（Prefill-Decode Disaggregation, PD）三种核心场景，
详细分析其通信需求的本质特征。
...
预填充-解码分离（Prefill-Decode Disaggregation, PD）是一种分布式推理架构优化，
...
随后从张量并行、专家并行与预填充-解码分离（PD）等典型场景出发，分析了 AllReduce、Dispatch/Combine 以及 KV Cache 传输等通信形态在数据规模与延迟敏感性上的差异。

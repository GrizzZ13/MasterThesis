...
在集群管理部分，参考现有工作\cite{blitzscale_paper,llumnix} 实现请求调度策略和模型自动扩缩容的逻辑。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/requests_per_second.png}
    \caption{BurstGPT 请求负载变化曲线}
    \label{fig:burstgpt-trace}
\end{figure}

图 ~\ref{fig:ttft-tbt-cdf} 展示了端到端测试场景下，
\sysname 与 NCCL 在 Time to First Token (TTFT) 和
Time Between Tokens (TBT) 两项指标上的累积分布函数（CDF）对比。
\sysname 和 NCCL 的 TTFT 和 TBT 中位数表现接近，
但 NCCL 在尾部表现出更大的波动，

在 TTFT 指标上，\sysname 相比于 NCCL 的 P50 和 P99 时延分别提升了 3.8\% 和 16\%。
对于 TTFT，由于系统需要进行 KV Cache 传输，Prefill 节点的计算与通信并行，
导致 NCCL 在部分请求上出现更长的 TTFT 时延。
在请求激增导致请求队列堆积、触发模型实例扩容时，Prefill 节点不作为模型参数源节点，因此不存在计算通信并行的情况；
但是由于 \sysname 相比于 NCCL 有更好的带宽表现，可以在更短的时间内完成模型参数分发，
请求排队的时间得以缩短，从而降低扩容过程对 TTFT 的影响。

在 TBT 指标上，\sysname 和 NCCL 的 P50 表现接近，
但 NCCL 在尾时延上出现了更大的波动，\sysname 的 P95 与 P99 时延相较于 NCCL 分别降低了 5.5\% 和 6.9\%。
TBT 波动主要来源于扩容过程中 Decode 节点计算与通信并发执行，导致 Decode 的性能收到影响。
并且由于 \sysname 能够更快地完成 KV Cache 传输，
因此第一个 Decode token 时延受到的影响较小，从而进一步降低了 TBT 的尾时延。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/ttft_tbt_cdf_pickle_vs_nccl.png}
...
图~\ref{fig:time-between-tokens} 展示了在不同通信实现下，
端到端推理过程中触发扩容的阶段 Time Between Tokens 性能。
...
在中位数接近的情况下，\sysname 能够显著改善尾部时延并降低 token 生成速率的波动。

...
点对点（P2P）通信是 \sysname 系统提供的基础能力之一，也是系统构建高性能数据传输通路的核心组件。
...
这主要是因为 \sysname 的控制面开销仅涉及 CPU 侧通过共享内存传输控制面信息，然后通过 Copy Engine 触发 NVLink 数据传输，数据面路径更贴近硬件；
相比之下 NCCL 采用了分片流水、GPU kernel同步、协议握手等机制，引入了一定的固定开销，使其无法完全利用所有的 NVLink 带宽资源；

值得注意的是，没有采取 IPC Handle 缓存优化的 \sysname 版本，在各个消息规模下的性能，
都显著低于 NCCL 和带有缓存优化的 \sysname 版本。
这主要是因为没有缓存优化的 \sysname 版本，在每次通信时都需要进行 \texttt{cudaIpcOpenMemHandle} 操作，
这一操作涉及用户态和内核态的切换，建立 GPU 内存页表映射等开销，这一系列开销所消耗的时间可以到达毫秒级；
尤其是映射的内存区域较大时需要映射的页数量众多，带来的开销进一步增大，从而显著影响传输性能。

\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/p2p_nvlink_bandwidth_vs_size.png}
    \bicaption{NVLink P2P 带宽随消息大小变化曲线}{NVLink P2P bandwidth vs. message size}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/p2p_rdma_bandwidth_vs_qp.png}
    \bicaption{RDMA P2P 有效带宽随 QP 数量变化曲线}{Effective bandwidth vs. number of QPs in RDMA P2P}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/p2p_rdma_bandwidth_vs_size.png}
    \bicaption{RDMA P2P 有效带宽随消息大小变化曲线}{Effective bandwidth vs. message size in RDMA P2P}
...
前台 Attention 计算部分选取 Qwen3-32B \cite{Qwen3-32B_model} 模型的注意力配置作为参数模拟真实推理场景，
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/p2p_nvlink_compute_impact_vs_size.png}
    \bicaption{NVLink P2P 相对计算性能随消息大小变化曲线}{Impact of message size on compute performance in NVLink P2P}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/p2p_rdma_compute_impact_vs_qp.png}
    \bicaption{RDMA P2P 相对计算性能随 QP 数量变化曲线}{Impact of number of QPs on compute performance in RDMA P2P}
...
图~\ref{fig:allgather-compute-impact} 展示了不同 AllGather 实现方式下的相对计算性能结果。
其中 baseline 表示无任何通信并发时的计算性能，
其余配置分别覆盖 NCCL 与 \sysname 在 RDMA 与 NVLink 链路上的 AllGather 性能。

\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/relative_perf_vs_msg_size_broken_y.png}
    \bicaption{NCCL AllGather 对计算性能的影响}{Impact of NCCL AllGather on compute performance}
    \label{fig:allgather-compute-impact}
\end{figure}
...
端到端测试面向模型即服务（Model-as-a-Service, MaaS）\cite{serverlessllm_paper,blitzscale_paper} 场景下的分布式大模型推理系统。
...
在此类场景下，通信对计算资源的占用情况将直接影响推理性能，通信与计算并发执行所带来的资源竞争会反映在 token 生成速率上。
因此，该测试场景能够有效放大通信机制差异，并检验系统在真实负载下的性能表现。

本实验基于 FlashInfer \cite{flashinfer_repo} 算子库，
使用 C++ 实现高性能的 PD 分离推理框架\cite{distserve_paper}，
其整体推理性能与当前学术界和工业界广泛采用的 SOTA 推理系统（如 vLLM）\cite{kwon2023pagedattention} 进行对齐，
并且分别使用 \sysname 和 NCCL 实现其点对点通信模块，用于 KV Cache 传输和通过网络传输进行的模型扩容。
在集群管理部分，参考现有工作\cite{blitzscale_paper,llumnix} 实现请求调度和自动扩缩容的策略。
选取 BurstGPT 作为请求负载\cite{burstgpt}，模拟 MaaS 请求动态变化的场景，
图 ~\ref{fig:burstgpt-trace} 展示了其请求负载的变化曲线。
在这个系统上，部署 Qwen3-32B \cite{Qwen3-32B_model} 模型，关注端到端的性能表现。

\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/requests_per_second.png}
    \bicaption{BurstGPT 请求负载变化曲线}{BurstGPT request load trace}
    \label{fig:burstgpt-trace}
\end{figure}


\subsection{端到端性能评测与分析}

图 ~\ref{fig:ttft-tbt-cdf} 展示了端到端测试场景下，
\sysname 与 NCCL 在 首令牌延迟 （Time to First Token，TTFT） 和
令牌间隔时间（Time Between Tokens，TBT） 两项指标上的累积分布函数（CDF）对比。
\sysname 和 NCCL 的 TTFT 和 TBT 中位数表现接近，
但 NCCL 在尾部表现出更大的波动，

在 TTFT 指标上，\sysname 相比于 NCCL 的 P50 和 P99 时延分别提升了 3.8\% 和 16\%。
对于 TTFT，由于系统需要进行 KV Cache 传输和模型参数传输，Prefill 节点的计算与通信并行，
导致 NCCL 在部分请求上出现更长的 TTFT 时延；
并且在请求激增导致请求队列堆积、触发模型实例扩容时，由于 \sysname 相比于 NCCL 有更好的带宽表现，
可以在更短的时间内完成模型参数分发，请求排队的时间得以缩短，从而降低扩容过程对 TTFT 的影响。

在 TBT 指标上，\sysname 和 NCCL 的 P50 表现接近，
但 NCCL 在尾时延上出现了更大的波动，\sysname 的 P95 与 P99 时延相较于 NCCL 分别降低了 5.5\% 和 6.9\%。
TBT 波动一方面来源于扩容过程中 Decode 节点计算与通信并发执行，导致 Decode 的性能受到影响；
另一方面在于选取 Prefill 节点作为模型参数传输源节点时，Prefill 节点存在模型参数传输与 KV Cache 传输的双重通信任务，
这两种通信任务存在带宽竞争，由于 NCCL 缺少细粒度的网络流量优先级传输机制，会导致部分 KV Cache 传输延迟增大，
而 \sysname 通过分片传输与优先级流量调度，有效缓解了该问题，从而进一步降低了 TBT 的尾时延。

\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/ttft_tbt_cdf_pickle_vs_nccl.png}
    \bicaption{端到端推理累积分布函数对比}{CDF comparison of end-to-end inference}
    \label{fig:ttft-tbt-cdf}
\end{figure}

\subsection{解码性能拆解分析}

为了对比不同通信实现下，端到端推理过程中触发扩容的阶段 Time Between Tokens 性能，
选取了扩容过程中相对稳态阶段的 TBT 性能进行拆解分析，具体如图~\ref{fig:time-between-tokens}所示。
在推理过程中，由于用户请求激增触发模型实例的在线扩容，
通过 NVLink 将模型参数从 Decode 节点分发至新实例。
...
即使在通信并发存在的情况下，Decode 阶段的计算性能仍能够保持相对稳定。

\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/time_between_tokens.png}
    \bicaption{扩容过程中的令牌生成间隔拆解}{Time Between Tokens during scaling process}
    \label{fig:time-between-tokens}
\end{figure}

\section{本章小结}
本章围绕 \sysname 在分布式大模型推理中\textbf{提升通信带宽并降低通信对计算的干扰}的核心目标进行了评测。
在点对点通信实验中，\sysname 在节点内 NVLink 与跨节点 RDMA 两类关键链路上均实现了稳定的高有效带宽，
并在不同消息尺度与并发度下表现出良好的适配性；
同时，由于 \sysname 采用控制面卸载与异步数据搬运机制，其通信过程对前台计算的 SM 资源占用显著更低，
从而在并发通信场景下保持了更高的计算稳定性。
在集合通信 AllGather 的干扰分析中，实验进一步表明：即使在更贴近真实 TP 推理负载的多 GPU 同步与数据交换形态下，
\sysname 仍能够有效实现计算通信解耦，降低集合通信引入的计算性能损失。
最后，在面向 MaaS 的端到端 PD 分离推理与在线扩容场景中，
本章从 TTFT 与 TBT 两项可观测指标出发验证了 \sysname 的实际收益：
在中位数接近的情况下，\sysname 能够显著改善尾时延并降低 token 生成速率的波动。
综上，\sysname 在微基准、干扰分析与端到端评测中均体现出性能优势，验证了其在分布式推理系统中的有效性与实用价值。

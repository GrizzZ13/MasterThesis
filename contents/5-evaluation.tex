...
本章以 NCCL 为基准，围绕 \sysname 在分布式大模型推理场景中 \textbf{提升通信带宽、降低通信对计算的干扰} 的核心目标开展实验与评测。
...
\todo{ref to https://docs.nvidia.com/dgx/dgxh100-user-guide/introduction-to-dgxh100.html}
\paragraph{硬件环境} 本章所涉及的测试均在 NVIDIA DGX H100 集群环境中进行。
该平台配备两颗 INTEL(R) XEON(R) PLATINUM 8558 处理器（共 96 物理核），能够为多 GPU 并行任务提供充足的主机侧算力与调度能力。
GPU 侧搭载 8 张 NVIDIA H100 GPU，总显存容量为 640\,GB。
在 GPU 互联方面，DGX H100 采用第 4 代 NVLink 互联（4th generation NVLink），为 GPU 间点对点通信提供最高约 900\,GB/s 的双向带宽。
...
\paragraph{软件环境} 本文所有测试所使用的操作系统为 Ubuntu 20.04 LTS。
其上使用的软件为 Python 3.12， CUDA 12.8，NCCL 2.25.1，GPU 驱动版本 570.195.03，OFED 版本 24.10。
...
还与协议栈开销、QP 并发度、在途请求数量（outstanding work requests）以及收发队列调度等因素密切相关。
...
选取 FlashAttention3 作为 Attention 算子实现，以获得 SOTA 的计算性能；
...
% AllGather 是张量并行（TP）等并行策略中常见的集合通信原语之一，
% 本节对比 NCCL 与 \sysname 在 NVLink 与 RDMA 两类链路上执行 AllGather 时对前台计算的影响。

% 图~\ref{fig:allgather-compute-impact} 总结了 AllGather 对计算性能的影响。
% 在 NVLink 场景下，通信并发会带来约 14\% 左右的计算下降，且 \sysname 与 NCCL 表现接近；
% 在 RDMA 场景下，NCCL 执行 all gather 集合通信会导致计算性能下降 11%，
% 而 \sysname 对计算影响均接近于零，说明跨节点通信在该测试设置下能够较好地与计算解耦。

\section{集合通信 AllGather 对计算的影响}

AllGather 是张量并行（Tensor Parallelism, TP）等并行策略中最为常见的集合通信原语之一，
...
端到端测试面向 MaaS（Model-as-a-Service）场景下的分布式大模型推理系统。
...
本实验基于 FlashInfer 算子库，使用 C++ 实现高性能的 PD 分离推理系统，
其整体推理性能与当前学术界和工业界广泛采用的 SOTA 推理系统（如 vLLM）进行对齐。
实验选取 Qwen3-32B 模型，重点关注解码（Decode）阶段的推理性能表现。

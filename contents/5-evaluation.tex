...
\section{本章小结}
本章围绕 \sysname 在大模型推理场景中的核心目标——提升通信带宽并降低通信对计算的干扰——开展了系统评测。
在点对点通信实验中，\sysname 在 NVLink 与 RDMA 两类链路上均能够提供稳定的高有效带宽，并在不同消息尺度下保持良好的伸缩性；
同时，相比 NCCL，\sysname 在节点内 NVLink 场景下显著降低了通信对前台计算的 SM 资源占用与性能干扰。
在集合通信 AllGather 的干扰分析中，实验进一步验证了 \sysname 在更贴近真实 TP 推理负载的通信形态下同样能够实现计算通信解耦。
最后，通过端到端 PD 分离推理场景的 TBT 指标评测，本章表明 \sysname 能够在真实负载中保持更稳定的解码节奏并改善尾延迟表现，
从而验证了控制面卸载与优先级调度等设计在实际推理系统中的有效性。

...
public:
  ...
  virtual ~Recver() = default;
    ...
    \end{minted}
    \caption{C++ Sender 与 Recver 接口定义}
...
数据传输、请求匹配、可见性保证等链路相关逻辑均由后续章节 \todo{ref} 介绍的 RDMA / NVLink 执行器负责。
...
并不意味着数据已经刷新到 GPU Memory 中。
因此 RDMA 链路执行器在初始化阶段基于环回 QP 构造了一个 Flusher，
用于主动触发 RDMA READ 以保证 PCIe ordering，从而确保 GPU 能看到之前 NIC 发起的写。
该 QP 的发送与接收都指向自己，通过对 GPU 地址执行 RDMA READ，
PCIe 会保证所有先前写入的数据在此 READ 之后对 GPU 可见。
...
\subsection{共享内存单生产者单消费者队列}

NVLink 链路围绕 CUDA IPC 提供的跨进程访问和 Peer Access功能构建。
...
将发送端内存映射到本进程的地址空间获取可以在进程内部直接访问的指针，
然后就可以在单进程内部完成 GPU-to-GPU 的内存拷贝了。

然而与 RDMA 场景中基于 \texttt{RcQueuePair} 进行控制面消息传递不同，
NVLink 控制面依赖共享内存单生产者单消费者队列，
用于在单节点内的多个进程之间传递 \texttt{NvlinkSendTicket} 与 \texttt{NvlinkSendTicket}，
其具体定义如图 ~\ref{fig:nvlink-control-plane-structure} 所示。

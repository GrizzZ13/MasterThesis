...
public:
  ...
  virtual ~Recver() = default;
    ...
    \end{minted}
    \bicaption{Sender 与 Recver 接口定义}{Sender and Recver interface definitions}
...
但执行器的内部逻辑（例如请求匹配、IPC Handle 以及地址偏移量交换、控制面队列的使用方式等）均对应用隐藏，
应用只需提供逻辑通道标识符（\texttt{unique\_id}）、缓冲区地址以及长度，即可提交一次完整的发送或接收请求。
...
recv_event->wait();
    \end{minted}
    \bicaption{应用侧发送与接收调用示例}{Example of send/recv calls in the application}
...
auto handle = runtime->start();
    \end{minted}
    \bicaption{运行时初始化示例}{Example of runtime initialization}
...
public:
  ...
  virtual ~Pollable() = default;
    ...
    \end{minted}
    \bicaption{可驱动对象接口定义}{Definition of the pollable interface}
    \label{fig:pollable-interface}
\end{figure}


在启动阶段，运行时根据构造器提供的 CPU 集合参数为每个后台线程设置线程亲和性。
对于涉及跨网卡和跨 NUMA 内存的执行器，这一策略可以避免执行器在轮询时发生跨 NUMA 节点的内存访问，
...
第二种是忙等待（busy wait）模式。在配置 \texttt{enable\_busy\_wait} 时，\texttt{wait()} 采用自旋方式，
...
public:
  ...
  void wait() {
    ...
    \end{minted}
    \bicaption{事件类核心结构和方法}{Definition and methods of the Event class}
...
在给定逻辑通道标识符（\texttt{unique\_id}）、起始地址与长度后，接口层立即为这个传输构造一个 \texttt{Event} 并返回。
之后的所有工作均转交给链路执行器，包括请求的匹配、调度推进与最终的完成通知。
在接收方向，\texttt{recv()} 采用完全对称的方式构造请求对象并交付底层处理。

采用 moodycamel ConcurrentQueue \cite{concurrentqueue_repo} 作为并发多生产者多消费者队列的实现，用于在接口层与执行器之间传递数据。
...
也不关心底层使用的是 GPUDirect RDMA 还是通过 NVLink 的进行跨 GPU 的内存拷贝。
...
上层的 RDMA 链路执行器模块 \texttt{executor\_rdma} 则利用这些抽象构建发送端 \texttt{RdmaSender}、
接收端 \texttt{RdmaRecver} 以及维护 GPUDirect RDMA 一致性的 \texttt{RdmaFlusher}。
...
图~\ref{fig:rdma-init} 给出了 RDMA 基础资源封装中核心抽象 \texttt{RcQueuePair} 的接口示意。
...
public:
  ...
  int poll_recv_cq_once(/*...*/) noexcept;
    ...
    \end{minted}
    \bicaption{RDMA 资源定义}{Definition and methods of RDMA resource}
    \label{fig:rdma-init}
\end{figure}

\paragraph{QP 握手与状态迁移}

可靠连接队列对（Reliable-Connection Queue Pair，RCQP）要进入可通信状态，必须经历 INIT $\rightarrow$ RTR $\rightarrow$ RTS 的三段状态迁移，
...
图~\ref{fig:memory-region-setup}展示了 MR 的创建过程示例。
...
// After that, buf can be used for RDMA operations
    \end{minted}
    \bicaption{内存区域创建示例}{Example of Memory Region creation}
    \label{fig:memory-region-setup}
\end{figure}

\paragraph{RdmaFlusher 实现}

GPUDirect RDMA 的完成语义需要额外区分“网卡侧完成”与“设备侧可见”：
...
RDMA 链路执行器在初始化阶段基于本地环回（loopback）的 RCQP 构造了一个 Flusher，该 QP 的发送与接收都指向自己；
...
图~\ref{fig:flusher-setup} 展示了 \texttt{RdmaFlusher} 的创建示例。
...
auto flusher = RdmaFlusher::create(std::move(qp)).unwrap();
    \end{minted}
    \bicaption{Flusher 创建流程示例}{Example of flusher creation}
...
负责从 RDMA 接收队列取出由接收端发来的 \texttt{RdmaTicket}，
并将其整理为按 \texttt{unique\_id} 逻辑通道分组的待匹配队列。具体流程如下：

首先调用 \texttt{poll\_recv\_cq\_once()} 批量轮询接收完成事件，然后对每个成功的 RDMA Recv，
从接收缓冲区中拷贝出对应的 \texttt{RdmaTicket}，
按照 \texttt{unique\_id} 为键插入到 \texttt{pending\_remote\_recv\_request\_map\_} 中，为后续的匹配与发送做准备，
同时立即向网卡接收队列重新投递 RDMA Recv 工作请求，保持网卡始终有充足槽位可以使用，在不阻塞 CQ 的前提下高效处理控制面请求。
...
\begin{figure}[!htp]
    ...
    \bicaption{工作请求标识符编码示意图}{wrid encoding}
    \label{fig:wrid-encoding}
\end{figure}

接收端 \texttt{RdmaRecver::poll()} 的运行逻辑与发送端对称，
其核心目标是在控制面与数据面之间推进接收路径的状态机。
该过程可分为三个阶段，对应算法~\ref{alg:rdma-recver-poll} 所示的批量命令提取、接收缓冲区发布以及完成事件处理。

首先，算法会尝试从并发队列中批量获取多条命令，命令数量上限为可配置参数 \texttt{kMagic}，
...
\begin{enumerate}
    ...
    \item 获取一个空闲的工作请求槽位；
    \item 在空闲槽位发布接收工作请求（RDMA Recv WR），用于接收发送端 RDMA Write with Imm 操作的立即数数据；
    \item 在空闲槽位发布发送工作请求（RDMA Send WR），将该 ticket 作为控制面消息发送给发送端。
\end{enumerate}
...
以通过额外的 RDMA Read 操作确保 GPUDirect RDMA 的可见性。
...
private:
  ...
  utils::Cache<cudaIpcMemHandle_t> handle_cache_;
...
// ...
private:
  /* Omit similar members as NvlinkSender but reversed roles */
  Queue<NvlinkCopyTask> copy_task_queue_;
  NvlinkCopier copier_;
};
    \end{minted}
    \bicaption{NVLink 链路执行器定义}{Definition of the NVLink executor}
    \label{fig:nvlink-executor}
\end{figure}

\subsection{跨进程通信内存访问机制}

NVLink 链路围绕 CUDA IPC 提供的跨进程访问和 Peer Access 功能构建\cite{nvidia2025cudipc,nvidia_nvlink_nvswitch}。
...
struct alignas(8) NvlinkAck {
  uint32_t unique_id;
    ...
    \end{minted}
    \bicaption{NVLink 链路执行器控制面元数据定义}{Definition of  NVLink executor control-plane metadata}
...
发送端首先调用 \texttt{send()} 发起通信请求，从本地缓存查找可用的 IPC Handle，
如果不存在，则根据配置情况决定是否调用 \texttt{cudaIpcGetMemHandle()} 创建新的 handle 并缓存。
获取到 IPC Handle 之后，将相对这个 handle 的起始地址偏移量、
...
构造函数在指定的 \texttt{device} 上创建 CUDA Context，并初始化一个专用的非阻塞 CUDA Stream；
...
通过将 CUDA IPC Memory Handle 的管理逻辑集中在 Copier 内部，\sysname 将跨进程 GPU 指针管理复杂度屏蔽，
保证了 handle 生命周期与数据传输严格绑定。

\subsection{IPC Handle 缓存优化}
...
如图 ~\ref{fig:cuda-ipc-overhead} 所示，打开/关闭 CUDA IPC Memory Handle 的延迟极高，
...
\begin{figure}[!htp]
    ...
    \bicaption{CUDA IPC Memory Handle 的打开/关闭开销}{Overhead of opening/closing CUDA IPC memory handles}
    \label{fig:cuda-ipc-overhead}
\end{figure}

\paragraph{缓存复用的优化思路}
针对上述瓶颈，\sysname 在数据面引入 IPC Handle 映射缓存：
...
并通过 \texttt{RdmaFlusher} 进一步保证 GPUDirect RDMA 场景下的数据可见性与一致完成语义；
本章也给出了 NVLink 执行器在节点内多 GPU 场景下的控制面队列组织、后台拷贝线程推进方式，
以及通过 IPC Handle 缓存复用降低 open/close 开销的关键优化。
